    ...
    std::call_once(once_flag, []()
            {
                ...
                factory->add_type_object("DistributedATS_Logout::Logout", DistributedATS_Logout::GetLogoutIdentifier(true),
                        DistributedATS_Logout::GetLogoutObject(true));
                factory->add_type_object("DistributedATS_Logout::Logout", DistributedATS_Logout::GetLogoutIdentifier(false),
                        DistributedATS_Logout::GetLogoutObject(false));
...
namespace DistributedATS_Logout {
    ...
    const TypeIdentifier* GetLogoutIdentifier(bool complete)
    {
        const TypeIdentifier * c_identifier = TypeObjectFactory::get_instance()->get_type_identifier("Logout", complete);
        if (c_identifier != nullptr && (!complete || c_identifier->_d() == EK_COMPLETE))
        {
            return c_identifier;
        }

        GetLogoutObject(complete); // Generated inside
        return TypeObjectFactory::get_instance()->get_type_identifier("Logout", complete);
    }

    const TypeObject* GetLogoutObject(bool complete)
    {
        const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("Logout", complete);
        ...
        {
            return GetCompleteLogoutObject();
        }
        //else
        return GetMinimalLogoutObject();
    }

    const TypeObject* GetMinimalLogoutObject()
    {
        const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("Logout", false);
        if (c_type_object != nullptr)
        {
            return c_type_object;
        }
        ...
        TypeObjectFactory::get_instance()->add_type_object("Logout", &identifier, type_object);
        delete type_object;
        return TypeObjectFactory::get_instance()->get_type_object("Logout", false);
    }

    const TypeObject* GetCompleteLogoutObject()
    {
        const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("Logout", true);
        ...
        // Header
        type_object->complete().struct_type().header().detail().type_name("Logout");
        // TODO inheritance
        TypeIdentifier identifier;
        ...
        TypeObjectFactory::get_instance()->add_type_object("Logout", &identifier, type_object);
        delete type_object;
        return TypeObjectFactory::get_instance()->get_type_object("Logout", true);
    }
