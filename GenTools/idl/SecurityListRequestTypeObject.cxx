...
/*!
 * @file SecurityListRequestTypeObject.cpp
...
void registerSecurityListRequestTypes()
{
    ...
    std::call_once(once_flag, []()
            {
                ...
                factory->add_type_object("DistributedATS_SecurityListRequest::SecurityListRequest", DistributedATS_SecurityListRequest::GetSecurityListRequestIdentifier(true),
                        DistributedATS_SecurityListRequest::GetSecurityListRequestObject(true));
                factory->add_type_object("DistributedATS_SecurityListRequest::SecurityListRequest", DistributedATS_SecurityListRequest::GetSecurityListRequestIdentifier(false),
                        DistributedATS_SecurityListRequest::GetSecurityListRequestObject(false));
...
namespace DistributedATS_SecurityListRequest {
    ...
    const TypeIdentifier* GetSecurityListRequestIdentifier(bool complete)
    {
        const TypeIdentifier * c_identifier = TypeObjectFactory::get_instance()->get_type_identifier("SecurityListRequest", complete);
        if (c_identifier != nullptr && (!complete || c_identifier->_d() == EK_COMPLETE))
        {
            return c_identifier;
        }

        GetSecurityListRequestObject(complete); // Generated inside
        return TypeObjectFactory::get_instance()->get_type_identifier("SecurityListRequest", complete);
    }

    const TypeObject* GetSecurityListRequestObject(bool complete)
    {
        const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("SecurityListRequest", complete);
        ...
        {
            return GetCompleteSecurityListRequestObject();
        }
        //else
        return GetMinimalSecurityListRequestObject();
    }

    const TypeObject* GetMinimalSecurityListRequestObject()
    {
        const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("SecurityListRequest", false);
        if (c_type_object != nullptr)
        {
            return c_type_object;
        }
        ...
        MinimalStructMember mst_SecurityReqID;
        mst_SecurityReqID.common().member_id(memberId++);
        mst_SecurityReqID.common().member_flags().TRY_CONSTRUCT1(false); // Unsupported
        mst_SecurityReqID.common().member_flags().TRY_CONSTRUCT2(false); // Unsupported
        mst_SecurityReqID.common().member_flags().IS_EXTERNAL(false); // Unsupported
        mst_SecurityReqID.common().member_flags().IS_OPTIONAL(false);
        mst_SecurityReqID.common().member_flags().IS_MUST_UNDERSTAND(false);
        mst_SecurityReqID.common().member_flags().IS_KEY(false);
        mst_SecurityReqID.common().member_flags().IS_DEFAULT(false); // Doesn't apply
        mst_SecurityReqID.common().member_type_id(*TypeObjectFactory::get_instance()->get_string_identifier(255, false));

        MD5 SecurityReqID_hash("SecurityReqID");
        for(int i = 0; i < 4; ++i)
        {
            mst_SecurityReqID.detail().name_hash()[i] = SecurityReqID_hash.digest[i];
        }
        type_object->minimal().struct_type().member_seq().emplace_back(mst_SecurityReqID);

        MinimalStructMember mst_SecurityListRequestType;
        mst_SecurityListRequestType.common().member_id(memberId++);
        mst_SecurityListRequestType.common().member_flags().TRY_CONSTRUCT1(false); // Unsupported
        mst_SecurityListRequestType.common().member_flags().TRY_CONSTRUCT2(false); // Unsupported
        mst_SecurityListRequestType.common().member_flags().IS_EXTERNAL(false); // Unsupported
        mst_SecurityListRequestType.common().member_flags().IS_OPTIONAL(false);
        mst_SecurityListRequestType.common().member_flags().IS_MUST_UNDERSTAND(false);
        mst_SecurityListRequestType.common().member_flags().IS_KEY(false);
        mst_SecurityListRequestType.common().member_flags().IS_DEFAULT(false); // Doesn't apply
        mst_SecurityListRequestType.common().member_type_id(*TypeObjectFactory::get_instance()->get_type_identifier("int32_t", false));


        MD5 SecurityListRequestType_hash("SecurityListRequestType");
        for(int i = 0; i < 4; ++i)
        {
            mst_SecurityListRequestType.detail().name_hash()[i] = SecurityListRequestType_hash.digest[i];
        }
        type_object->minimal().struct_type().member_seq().emplace_back(mst_SecurityListRequestType);
        ...
        TypeObjectFactory::get_instance()->add_type_object("SecurityListRequest", &identifier, type_object);
        delete type_object;
        return TypeObjectFactory::get_instance()->get_type_object("SecurityListRequest", false);
    }

    const TypeObject* GetCompleteSecurityListRequestObject()
    {
        const TypeObject* c_type_object = TypeObjectFactory::get_instance()->get_type_object("SecurityListRequest", true);
        ...
        CompleteStructMember cst_SecurityReqID;
        cst_SecurityReqID.common().member_id(memberId++);
        cst_SecurityReqID.common().member_flags().TRY_CONSTRUCT1(false); // Unsupported
        cst_SecurityReqID.common().member_flags().TRY_CONSTRUCT2(false); // Unsupported
        cst_SecurityReqID.common().member_flags().IS_EXTERNAL(false); // Unsupported
        cst_SecurityReqID.common().member_flags().IS_OPTIONAL(false);
        cst_SecurityReqID.common().member_flags().IS_MUST_UNDERSTAND(false);
        cst_SecurityReqID.common().member_flags().IS_KEY(false);
        cst_SecurityReqID.common().member_flags().IS_DEFAULT(false); // Doesn't apply
        cst_SecurityReqID.common().member_type_id(*TypeObjectFactory::get_instance()->get_string_identifier(255, false));

        cst_SecurityReqID.detail().name("SecurityReqID");

        type_object->complete().struct_type().member_seq().emplace_back(cst_SecurityReqID);

        CompleteStructMember cst_SecurityListRequestType;
        cst_SecurityListRequestType.common().member_id(memberId++);
        cst_SecurityListRequestType.common().member_flags().TRY_CONSTRUCT1(false); // Unsupported
        cst_SecurityListRequestType.common().member_flags().TRY_CONSTRUCT2(false); // Unsupported
        cst_SecurityListRequestType.common().member_flags().IS_EXTERNAL(false); // Unsupported
        cst_SecurityListRequestType.common().member_flags().IS_OPTIONAL(false);
        cst_SecurityListRequestType.common().member_flags().IS_MUST_UNDERSTAND(false);
        cst_SecurityListRequestType.common().member_flags().IS_KEY(false);
        cst_SecurityListRequestType.common().member_flags().IS_DEFAULT(false); // Doesn't apply
        cst_SecurityListRequestType.common().member_type_id(*TypeObjectFactory::get_instance()->get_type_identifier("int32_t", false));


        cst_SecurityListRequestType.detail().name("SecurityListRequestType");

        type_object->complete().struct_type().member_seq().emplace_back(cst_SecurityListRequestType);
        ...
        // Header
        type_object->complete().struct_type().header().detail().type_name("SecurityListRequest");
        // TODO inheritance
        TypeIdentifier identifier;
        ...
        TypeObjectFactory::get_instance()->add_type_object("SecurityListRequest", &identifier, type_object);
        delete type_object;
        return TypeObjectFactory::get_instance()->get_type_object("SecurityListRequest", true);
    }
