...
;
	...
	if (grp.isSetField(FIX::FIELD::Symbol) )
		ddsMsg.c_NoMDEntries[NoMDEntries_group_cnt_index].Symbol = CORBA::string_dup(((FIX::Symbol)grp.getField(FIX::FIELD::Symbol)).getString().c_str());

	if (grp.isSetField(FIX::FIELD::SecurityExchange) )
		ddsMsg.c_NoMDEntries[NoMDEntries_group_cnt_index].SecurityExchange = CORBA::string_dup(((FIX::SecurityExchange)grp.getField(FIX::FIELD::SecurityExchange)).getString().c_str());
...
;
	...
	DistributedATS::convert_dds_string_to_fix(ddsMsg.c_NoMDEntries[NoMDEntries_group_cnt_index].Symbol.in(), FIX::FIELD::Symbol, grp);

	DistributedATS::convert_dds_string_to_fix(ddsMsg.c_NoMDEntries[NoMDEntries_group_cnt_index].SecurityExchange.in(), FIX::FIELD::SecurityExchange, grp);
